...
- **State Transitions**:
  ...
  - `HALF-OPEN`: After the 60s cooldown, exactly *one* operation is permitted to attempt reaching Redis. If it succeeds, the breaker resets to `CLOSED`. If it fails, the breaker returns to `OPEN` for another 60s.
